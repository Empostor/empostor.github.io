...
Empostor has an Http Server that is used by recent versions of Among Us to connect to. See [the Http Server page](reverse-proxy.md) for more details on how to set this up.
...
Send game events to Discord channels via webhooks. Events are split into two categories: matchmaker (game lifecycle) and admin (moderation). See [Discord Webhook](../plugins/discord-webhook.md) for details.
...
These are **plugin** settings and do not live in `config.json`. The [Player Stats](../plugins/player-stats.md) and [Chat Filter](../plugins/chat-filter.md) plugins each read their own config file next to the server executable:

- `[Empostor.Plugins.PlayerStats]Config.json`
- `[Empostor.Plugins.ChatFilter]Config.json`

See each plugin page for the field reference. Chat Filter settings changed via the Admin Panel are runtime-only and are not written back to the file.
