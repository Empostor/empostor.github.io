...
To connect to your Among Us version from another PC, you need to set up a HTTP reverse proxy to support HTTPS. If you're just running a version of Empostor for testing Setup instructions for this are in the [Http Server documentation](reverse-proxy.md).
...
3. Extract the zip.
4. Modify `config.json` to your liking. Documentation can be found [here](configuration.md). You need to at least change `PublicIp` to the address people will connect to your server to.
5. Run `Empostor.Server` (Linux/macOS) or `Empostor.Server.exe` (Windows)
6. Set up a reverse proxy to support HTTPS, so you can connect to your server from another device. See [reverse proxy configuration](reverse-proxy.md)
7. (OPTIONAL - Linux) Configure a systemd definition file and enable the service to start on boot, see [systemd configuration](configuration.md#systemd)
