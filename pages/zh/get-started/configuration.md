...
Empostor 包含一个 HTTP 服务器，供新版 Among Us 用于连接。详见 [HTTP 服务器页面](reverse-proxy.md)。
...
通过 Webhook 将游戏事件发送到 Discord 频道。事件分为两个类别：对局（游戏生命周期）和管理（审核）。详见 [Discord 通知](../plugins/discord-webhook.md)。
...
这两项属于**插件**设置，不在 `config.json` 中。[玩家统计](../plugins/player-stats.md)与[聊天过滤](../plugins/chat-filter.md)插件各自读取服务器可执行文件旁自己的配置文件：

- `[Empostor.Plugins.PlayerStats]Config.json`
- `[Empostor.Plugins.ChatFilter]Config.json`

字段说明见各自插件页面。通过管理面板修改的聊天过滤设置为运行时生效，不会写回文件。
