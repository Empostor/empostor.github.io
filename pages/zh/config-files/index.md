...
主服务器配置文件。完整参考请参阅[服务器配置](../get-started/configuration.md)。
...
`AuthApi` 部分控制 Empostor 如何获取玩家好友代码以进行身份验证。完整流程请参阅[验证 API](../reference/verification-api.md)。
...
更多详情请参阅 [Hello 页面](../server/hello-page.md)。
...
关于代码层面的插件多语言支持（`IPluginLanguageProvider`），请参阅[编写插件](../develop/writing-a-plugin.md#国际化-i18n)。
...
定义管理面板插件市场中可用的插件。完整格式参考请参阅[插件市场](../server/plugin-marketplace.md)。
